// src/admin-sections/Doctors/DoctorForm.jsx
import React, { useState, useEffect } from 'react';
import { Eye, EyeOff } from 'lucide-react';

const DoctorForm = ({ doctorToEdit, onSave, onCancel }) => {
  const [formData, setFormData] = useState({
    ...
    gender: '',
    mobile: '',
    department_id: '', // سيتم استبدالها بقائمة منسدلة حقيقية
    specialty_id: '',  // سيتم استبدالها بقائمة منسدلة حقيقية
    rating: '',
    description: '',
    birth_date: '',
    hire_date: '',
    image: 'https://placehold.co/200x200/CCCCCC/FFFFFF?text=Doctor', // صورة افتراضية
  });
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h3 className="text-2xl font-semibold text-gray-800 mb-6 text-right">
        {doctorToEdit ? 'تعديل بيانات الطبيب' : 'إضافة طبيب جديد'}
      </h3>
      <form onSubmit={handleSubmit} className="space-y-4 text-right">
        ...
        <div className="relative">
          ...
          <input
            ...
            required={!doctorToEdit} // مطلوب عند الإضافة فقط
          />
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="rating" className="block text-gray-700 text-sm font-bold mb-2">التقييم</label>
            <select id="rating" name="rating" value={formData.rating} onChange={handleChange} className="shadow border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right appearance-none bg-white pr-8" required>
        ...
        <div>
          <label htmlFor="image" className="block text-gray-700 text-sm font-bold mb-2">صورة الطبيب (URL)</label>
        ...
        <div className="flex justify-end space-x-4 space-x-reverse">
          ...
          >
            {doctorToEdit ? 'حفظ التعديلات' : 'إضافة'}
          </button>
          <button
            type="button"
            onClick={onCancel}
